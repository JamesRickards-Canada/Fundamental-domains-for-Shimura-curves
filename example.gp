...
X=afuchinit(A, , , , 0);\\Initialize the Fuchsian group structure
U=afuchfdom(X);\\Compute the fundamental domain
S=afuchsignature(X);\\Signature
P=afuchpresentation(X);\\Presentation
elt=algmulvec(A, U[1], [1,2,7,1,2,2,1,5,7]);\\Generating an element of the group (happens to be hyperbolic)
