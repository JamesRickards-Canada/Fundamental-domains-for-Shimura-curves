...
\\Quaternion methods
	...
	install("algfdom","GGD1,L,D1,L,D0,G,D0,G,D0,L,p","algfdom","./libfdom.so");
	addhelp(algfdom,"Inputs A, p, {dispprogress=1}, {dumppartial=1}, {partialset=0}, {CNRdata=0}, {type=0}: quaternion algebra A split at one real place, upper half plane point p, dispprogress=0,1, dumppartial=0,1, partialset=0 or a set of norm 1 elements of A, ANRdata=0 or a length 5 vector, type=0, 1, 2.\n Computes and returns the fundamental domain for the group of units of norm 1 in A. We use the unit disc model, which the upper half plane is mapped to via p->0. p need to NOT be a fixed point of this group under the standard embedding into PSL(2, R) (p=I/2 is safe for quaternion algebras over Q). If dispprogress=1, displays partial progress. If dumppartial=1, this dumps generating sets of partial results to 'algfdom_partialdata_log.txt', where i is a number. This is useful in case of error, for example running out of memory, lack of precision, segmentation fault, etc. partialset is either 0 or a vector of norm 1 elements of A that can be used as a starting base for the fundamental domain. CNRdata is a technical entry, and can be passed in to specify some/all constants used in the enumeration. If type=1, we enumerate using qfminim. If type=2, we enumerate using the `improved' Fincke-Pohst method. If type=0, we automatically choose type 1 if n>=2 and type 2 if n=1 (which seems to be the optimal choice).");
