...
/*SECTION 1: GEOMETRIC METHODS*/
  ...
  install(disc_act,"GGp",,fdom_library);
  addhelp(disc_act,"disc_act(M, z): returns the action of M on z, where we are working in the unit disc model. M=[A, B] with |A|^2-|B|^2=1 acts on the unit disc model via the normal Mobius action of [A, B;conj(B), conj(A)].");
  install(klein_act,"GGp");
  addhelp(klein_act,"klein_act(M, z): returns the action of M on z, where we are working in the Klein model. M=[A, B] with |A|^2-|B|^2=1 acts on the unit disc model via Mz=(A^2z+B^2conj(z)+2AB)/(|A|^2+|B|^2+A*z*conj(B)+conj(A)*conj(z)*B.");
  
  /*1: TRANSFER BETWEEN MODELS*/
  install(disc_to_klein_user,"Gp",disc_to_klein);
  addhelp(disc_to_klein,"disc_to_klein(z): moves the complex point z from the unit disc model to the Klein model.");
  install(klein_to_disc_user,"Gp",klein_to_disc);
  addhelp(klein_to_disc,"klein_to_disc(z): moves the complex point z from the Klein model to the unit disc model.");
...
/*SECTION 3: QUATERNION ALGEBRA METHODS*/
  ...
  addhelp(afuchelliptic,"afuchelliptic(X): retrieves the elliptic elements of X. Their orders appear from smallest to largest, and can be retrieved with afuchsignature(X)[2].");
  install(afuchelts,"G");
...
/*SECTION 1: VISUALIZATION*/
  ...
  addhelp(afuchfdom_pmovie,"afuchfdom_pmovie(X, dat, filename, {model=1}, {boundcircle=1}, {compile=1}, {open=1}): makes a movie of the Dirichlet domain computed with respect to a moving centre p. The input dat supplies the points p: either a vector of points in the upper half plane, or dat=[N, z] representing N equally spaced points chosen in a hyperbolic circle with centre I and going through z. The results are saved to ./pmovie/filename.pdf.");
