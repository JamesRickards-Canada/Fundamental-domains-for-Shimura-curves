...
\\Quaternion methods
	...
	install("algfdomreduce","GGGDGD0,G,p","algfdomreduce","./libfdom.so");
	addhelp(algfdomreduce,"Inputs g, U, A, {O=NULL}, {z=0}: quaternion algebra A split at one real place with fundamental domain (of the Eichler order O) U, an element g of this group, and a point z in the unit disc.\n Returns the triple [gammabar, delta, decomp], where gammabar=delta*g is (G,z)-reduced (i.e. distance between gammabar*z and 0 is less than or equal to the distance between g'*gammabar*z for all g' in G), and decomp is the vecsmall [i1, i2, ..., in] with delta=G[i1]*G[i2]*...*G[in]. If z=0, then gammabar=+/-1.");
	install("algfdomrootgeodesic","GGGDGp","algfdomrootgeodesic","./libfdom.so");
	addhelp(algfdomrootgeodesic,"Inputs g, U, A, {O=NULL}: quaternion algebra A split at one real place with fundamental domain (of the Eichler order O) U, an element g of this group.\n Returns the root geodesic of g in the fundamental domain. The format is [g's, circle arcs, vecsmall(sides hit), vecsmall(sides emenating from)].");
	...
	install("algfdomword","GGGGDGp","algfdomword","./libfdom.so");
	addhelp(algfdomword, "Inputs: g, P, U, A, {O=NULL}: quaternion algebra A split at one real place with fundamental domain (of the Eichler order O) U, presentation P (given with algfdompresentation), and element g of norm 1.\n This writes g as a word in P[1], and returns the word. The format is Vecsmall([i1, i2, ..., ik]) corresponds to the equation V[1][|i1|]^{sign(i1)}*...*V[1][|ik|]^{sign(ik)} (i.e. g_1^2g_2^-1->[1, 1, -2]).");
