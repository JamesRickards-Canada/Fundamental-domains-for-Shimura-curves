...
\\GEOMETRY
	...
	install("hdiscrandom","Gp","hdiscrandom",fdom_library);
	...
	install("hdist","GGD0,L,p","hdist",fdom_library);
...
\\Visualization
	...
	install("fdom_latex","vGrD1,L,D1,L,D1,L,p", ,fdom_library);
	...
	install("python_plotviewer","vr","python_plotviewer",fdom_library);
	...
	install("python_printarcs","vGrD0,L,Drp","python_printarcs",fdom_library);
	...
	install("python_printfdom","vGrp","python_printfdom",fdom_library);
...
\\Quaternion methods
	...
	install("algfdom","GDGD0,L,D0,G,p","algfdom",fdom_library);
	addhelp(algfdom,"Inputs A, {O=NULL}, {dispprogress=0}, {constants=0}: quaternion algebra A split at one real place, O an Eichler order (4n x 4n Z-matrix written in the basis of the stored maximal order, columns spanning the order), dispprogress=0,1, constants=0 or [p, C, R, passes, type].\n Computes and returns U, the fundamental domain for the group of units of norm 1 in O, where O is set to the pre-computed maximal order in A if not supplied. U[1] is the vector of elements corresponding to the sides of the domain, and U[7] is the side pairing. We use the unit disc model, which the upper half plane is mapped to via p->0. p need to NOT be a fixed point of this group under the standard embedding into PSL(2, R) (p=I/2 is safe for quaternion algebras over Q). If dispprogress=1, displays partial progress. If any of p, C, R, passes, or type is 0, it is auto-set. C is the constant used in solving Q_{z_1, z_2}(x)<=C, R is the radius we choose random points from, passes is used to specify the estimated number of passes (larger=less points per pass). If type=1, we enumerate using qfminim. If type=2, we enumerate using the `improved' Fincke-Pohst method. If type=0, we automatically choose type 1 if n>=2 and type 2 if n=1 (which seems to be the optimal choice).");
	install("algfdomalg","G","algfdomalg",fdom_library);
	...
	install("algfdomarea","GDGD0,L,p","algfdomarea",fdom_library);
	...
	install("algfdomminimalcycles","Gp","algfdomminimalcycles",fdom_library);
	...
	install("algfdomorder","G","algfdomorder",fdom_library);
	addhelp(algfdomorder,"Input U, a fundamental domain.\n Returns the order associated to the fundamental domain.");
	install("algfdompresentation","Gp","algfdompresentation",fdom_library);
	...
	install("algfdomrootgeodesic","GGp","algfdomrootgeodesic",fdom_library);
	...
	install("algmulvec","GGG","algmulvec",fdom_library);
	...
	install("algnormdisc","G","algnormdisc",fdom_library);
	addhelp(algnormdisc,"Input A, an algebra.\n Returns the norm to Q of the discriminant of A.");
	install("algorderdisc","GGD1,L,D1,L,","algorderdisc",fdom_library);
	...
	install("algorderlevel","GGD1,L,","algorderlevel",fdom_library);
	...
	\\install("algfdom_bestC","GDGp","algfdom_bestC",fdom_library);
	...
	\\install("algnormalizedbasis","GDGGGp","algnormalizedbasis",fdom_library);
	...
	\\install("algnormalizedboundary","GDGGGp","algnormalizedboundary",fdom_library);
	...
	\\install("algfdomreduce","GGD0,G,p","algfdomreduce",fdom_library);
	...
	\\install("algramifiedplacesf","G","algramifiedplacesf",fdom_library);
	\\addhelp(algramifiedplacesf,"Input A, an algebra.\n Returns the vector of finite places that ramify.");
	\\install("algabsrednorm","GGD0,G,D0,G,p","algabsrednorm",fdom_library);
	...
	\\install("algsmallnorm1elts","GDGGGD0,G,D0,G,D0,L,p","algsmallnorm1elts",fdom_library);
	...
	\\fdom_extra
	install("algsmallelts","GDGGGGD0,G,D0,G,p","algsmallelts",fdom_library);\\Still working on this.
	
	install("alg1ijktoalg","GG","alg1ijktoalg",fdom_library);
	...
	install("alg1ijktobasis","GG","alg1ijktobasis",fdom_library);
	...
	install("algbasisto1ijk","GG","algbasisto1ijk",fdom_library);
	addhelp(algbasisto1ijk,"Inputs: quaternion algebra A, element x in basis form.\n Returns [e, f, g, h], where x=e+fi+gj+hk.");
	install("algeichler_conj","GG","algeichler_conj",fdom_library);
	install("algeichler1", "vGG", , fdom_library);
	...
	install("algeichler2","GG", , fdom_library);
	...
	install("algorderconj","GGDG","algorderconj",fdom_library);
	...
	install("algreduceddisc","G","algreduceddisc",fdom_library);
	...
	install("algshimura","GGD1,L,D20,L,D1,L,","algshimura",fdom_library);
	...
	install("algshimura_ab","GGD1,L,D1,L,","algshimura_ab",fdom_library);
	...
	install("algswapab","G","algswapab",fdom_library);
	addhelp(algswapab,"Input A, a quaternion algebra=(a, b/F).\n Returns (b, a/F), i.e. swapping a and b.");
	install("smallalgebras","GLD2,G,D0,G,D20,L,D0,L,","smallalgebras",fdom_library);
	...
	install("smallalgebras_area","GGGD0,L,D20,L,D1,L,p","smallalgebras_area",fdom_library);
...
\\PAPER METHODS
	...
	install("enum_bestC","GGGLD300,L,p","enum_bestC",fdom_library);
	...
	install("enum_bestC_range","GGGLLsD0,L,D1,L,D1,L,p","enum_bestC_range",fdom_library);
	...
	install("enum_successrate","GGGLD0,G,p","enum_successrate",fdom_library);
	...
	install("enum_successrate_range","GGGGLLD0,G,DsD1,L,D1,L,p","enum_successrate_range",fdom_library);
	...
	install("enum_time","GGGD300,L,p","enum_time",fdom_library);
	...
	install("enum_time_range","GGGGLD300,L,DsD1,L,D1,L,p","enum_time_range",fdom_library);
	...
	install("enum_timeforNelts","lGGGLD0,G,D1,L,p","enum_timeforNelts",fdom_library);
	...
	install("enum_timeforNelts_range","vGGGGLLsD1,L,D1,L,D1,L,p","enum_timeforNelts_range",fdom_library);
	...
	install("algfdom_nelts","GGD0,G,D0,L,p","algfdom_nelts",fdom_library);
	...
	\\REGRESSIONS & PLOTS
	install("OLS","GGD1,L,","OLS",fdom_library);
	...
	install("OLS_nointercept","GGD1,L,","OLS_nointercept",fdom_library);
	...
	install("OLS_single","GGD1,L,","OLS_single",fdom_library);
	...
	install("rsquared","GGG","rsquared",fdom_library);
	...
	install("plot_compile","vsD1,L,","plot_compile",fdom_library);
